...
#函数功能：计算当前的梯度
def calGradient(m, n, theta, order):
	gra = []
...
#函数功能：实现共轭梯度放方法
def conjugateGradient(t, s, weight, max_power):
	
	residue = -calGradient(t, s, weight, max_power)#计算最速下降方向
	...
	while i<iMax and gra0 > e*beta0:
		...
		while j<jMax and alpha**2*norm>e:#线性迭代，使得在该方向取得最小值
			#计算步长alpha
			alpha = -numpy.dot(calGradient(t, s, renew_theta, max_power).T,direction) / (numpy.dot(direction.T,numpy.dot(Hes(t, max_power),direction)))
#			print "alpha=", alpha
			renew_theta = renew_theta + alpha * direction#theta的迭代方程
			j += 1
		
		#计算用于迭代搜索方向的参数beta
		residue = -calGradient(t, s, renew_theta, max_power)#计算当前点的最速下降方向
		gra1 = gra0
		...
		if count==iterUp or numpy.dot(residue.T,direction)<=0:
			...
			count = 0
		print '------'
		tmp=lossFun(X,Y,renew_theta,order)
		print 'J=',tmp,', count=',i
...
#利用共轭梯度法进行拟合
theta=conjugateGradient(X, Y, theta,order)
#画出拟合后的曲线
